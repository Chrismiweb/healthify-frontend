...
const getIcon = (name, className = "w-6 h-6") => {
  ...
  const IconComponent = iconMap[name] || settings; // fallback icon
...
const Sidebar = ({ activeTab, setActiveTab, isOpen, toggleSideBar, width }) => {
  return (
      ...
      {width > 1230 ? (
        ...
        <div className="w-full">
          <div className="p-6 border-b">
            <img src="/logo.png" alt="logo" className="w-auto h-auto"></img>
          </div>
          <ul className="m-6">
            {tabs.map((tab) => (
              <li 
                ...
                className={`flex items-center gap-3 p-3 m-2 rounded-lg cursor-pointer transition-all duration-200 hover:bg-gray-50 ${
                  activeTab === tab.id ? 'bg-blue-50 text-blue-600 border-r-4 border-blue-600' : 'text-gray-700'
                }`}
